...
var emojiRE = regexp.MustCompile(emojiiPattern)

func getKeyword(str string) string {
	emojis := emojiRE.FindAllString(str, 8)
	if len(emojis) > 0 {
	...
	for _, word := range strings.Fields(str) {
		...
		if !punctuation && len(word) > 1 {
			...
			if unicode.IsUpper(letters[0]) {
				capitalized = append(capitalized, word)
			}
		}
		punctuation = strings.ContainsAny(word, ".?!")
	}
	if len(capitalized) > 0 {
		longest = ""
		for _, word := range capitalized {
			if len(word) > len(longest) {
				longest = word
			}
		}
